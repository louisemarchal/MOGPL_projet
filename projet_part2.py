    ...
    for i in range(N) :
        for t in range(len(seqligne[i])):
            for j in range(M):
                ...
                #lastk = min(j+seqligne[i][t], M)
                #m.addConstr(y[i][t][j]*seqligne[i][t] - quicksum([x[i][k] for k in range(j,lastk)]) <= 0)
                compt+=1
    ...
    for j in range(M) :
        for t in range(len(seqcol[j])):
            for i in range(N):
                ...
                #lastk = min(i+seqcol[j][t], N)
                #m.addConstr(z[j][t][i]*seqcol[j][t] - quicksum([x[k][j] for k in range(i,lastk)]) <= 0)
                compt+=1
